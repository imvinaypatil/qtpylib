...
# -*- coding: UTF-8 -*-
#
# webull: a Pythonic Client for webull API
...
from ezibpy import dataTypes
from ezibpy.utils import local_to_utc
...
from qtpylib import tools, asynctools
from dateutil.parser import parse as parse_date
from webull import webull as wb
from webull import paper_webull, streamconn
...
class webull():
    ...
    def __init__(self, paper=False):
        ...
        self.streamConnection = StreamConn(debug_flg=True)
        ...
        # -------------------------------------
        self.log = logging.getLogger('webull')  # get logger
        # -------------------------------------
    ...
    def disconnect(self):
        if self.connected and self.wb is not None:
            ...
            self.wb.logout()
            self.streamConnection.disconnect()
            self._disconnected_by_user = True
            self.connected = False
            self.started = False
            self.handleServerEvents(topic="CONNECTION_CLOSED", msg="connection closed externally", data=None)
    ...
    def reconnect(self):
        ...
        if self.started:
            ...
            self.streamConnection.disconnect()
            self.start()
        self.handleServerEvents(topic="CONNECTION_RESET", msg="connection refreshed", data=None)
    ...
    def handleServerEvents(self, topic, data, msg=None):
        ...
        #     self.callbacks(caller="handleTickSnapshotEnd", msg=msg)
        # else:
        #     # log handler msg
        ...
        if duplicateMessage is False:
            ...
            for accountCode in self.accountCodes:
                self.account_symbols_orders[accountCode] = self.group_orders(
                    "symbol", accountCode)
            self.callbacks(caller="handleOrders", msg=msg)
    ...
    def handleMarketDepth(self, msg):
        ...
        if data['deal']['trdBs'] == 'B':
            self.marketDepthData[tickerId].loc[0, "bid"] = data['deal']['price']
            self.marketDepthData[tickerId].loc[0, "bidsize"] = data['deal']['volume']

        # ask
        elif data['deal']['trdBs'] == 'S':
            self.marketDepthData[tickerId].loc[0, "ask"] = data['deal']['price']
            self.marketDepthData[tickerId].loc[0, "asksize"] = data['deal']['volume']
    ...
    def handleHistoricalData(self, msg, tickerId, completed):
        ...
        if symbol not in self.historicalData.keys():
            ...
            try:
                ...
                self.log.warning('discarded duplicate rows')
    ...
    def createContract(self, contractTuple, **kwargs):
        ...
        print('CONTACT_STRING', contractString)
    ...
    def cancelMarketDepth(self, contracts=None):
        ...
        for contract in contracts:
            tickerId = self.tickerId(self.contractString(contract))
            self.wb.cancelMktDepth(tickerId=tickerId)
    ...
    def subscribeMarketData(self, contracts=None, snapshot=False):
        ...
        for contract in contracts:
            ...
            if not self.isMultiContract(contract):
                tickerId = self.tickerId(self.contractString(contract))
                self.streamConnection.subscribe(tId=tickerId)
    ...
    def requestPositionUpdates(self, subscribe=True):
        ...
        if self.subscribePositions != subscribe:
            ...
            if subscribe == True:
                self.wb.reqPositions()
            else:
                self.wb.cancelPositions()
